...
const ViewOfficers = () => {
  ...
  useEffect(() => {
    ...
    // get officers from localhost officersData
    const officersData = localStorage.getItem("officersData");
    if(officersData){
      setResults(JSON.parse(officersData).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)) || []);
    }
  ...
  //     axios
  //       .get(`${process.env.REACT_APP_API_URL}/officers`, { headers })
  //       .then((response) => {
  //         console.log(response.data);
  //         const sortedOfficers = response.data.data.sort(
                                        ...
                                        <tr key={key}>
                                          <td><div className="float-left p-2">
                                            ...
                                            <td>
                                                ...
                                                <StatusModal
                                                  title={`${result?.status === "active" ? "Disable" : "Enable"} Officer`}
                                                  content={
                                                    `Are you sure you want to ${result?.status === "active" ? "disable" : "enable"} this officer?`
                                                  }
